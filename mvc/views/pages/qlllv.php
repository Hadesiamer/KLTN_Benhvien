...
// Dữ liệu nghỉ phép lấy từ controller (MVC)
$dsNghiPhep = isset($data["NghiPhep"]) ? json_decode($data["NghiPhep"], true) : [];
$count = isset($data["SoYCNghiPhep"]) ? (int)$data["SoYCNghiPhep"] : 0;
...
// ================== KHÔNG CÒN KẾT NỐI DB TRONG VIEW ==================
...
// Tạo ngày trong tuần (dùng để hiển thị)
for ($i = 1; $i <= 7; $i++) {
    ${'t' . ($i + 1)} = date('d-m-Y', strtotime($ngaydautuan . " +$i days"));
}

// Tạo mảng ngày trong tuần (dùng để so sánh với dữ liệu lịch)
$daysOfWeek = [];
for ($i = 1; $i < 8; $i++) {
    $daysOfWeek[] = date('Y-m-d', strtotime($ngaydautuan . " +{$i} days"));
}

// ===== PHẦN HIỂN THỊ LỊCH LÀM VIỆC =====
echo '<h2 class="text-center mb-4" style="background-color: #007bff; color: white; font-weight: bold; padding: 5px; border-radius: 5px;">Quản lý lịch làm việc bác sĩ</h2>';
...
foreach ($daysOfWeek as $day) {
    echo '<td class="shift-cell morning editable-cell" data-day="' . $day . '" data-shift="Sáng">';
    foreach ($dt as $data) {
    ...
    // Thêm lịch làm việc ở từng ngày cụ thể (chỉ cho phép sau hôm nay + 2 ngày)
...
foreach ($daysOfWeek as $day) {
    echo '<td class="shift-cell afternoon editable-cell" data-day="' . $day . '" data-shift="Chiều">';
    foreach ($dt as $data) {
                  ...
                  <tr>
                    ...
                    <td>
                      <form method="POST" action="" class="d-inline">
                        <!-- Dùng MaYC + MaLLV để xử lý chính xác -->
                        <input type="hidden" name="MaYC" value="<?php echo (int)$row['MaYC']; ?>">
                        <input type="hidden" name="MaLLV" value="<?php echo (int)$row['MaLLV']; ?>">
                        <input type="hidden" name="MaNV" value="<?php echo htmlspecialchars($row['MaNV']); ?>">
                        <input type="hidden" name="NgayLamViec" value="<?php echo htmlspecialchars($row['NgayLamViec']); ?>">
                        <input type="hidden" name="CaLamViec" value="<?php echo htmlspecialchars($row['CaLamViec']); ?>">
